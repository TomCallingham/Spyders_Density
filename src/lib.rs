...
fn epanechnikov_kde_py<'py>(
    ...
    let res = match n_dim {
        1 => kde_funcs::epanechnikov_kde::<1>(x, points, lamdaopt, n_threads),
        2 => kde_funcs::epanechnikov_kde::<2>(x, points, lamdaopt, n_threads),
        3 => kde_funcs::epanechnikov_kde::<3>(x, points, lamdaopt, n_threads),
        4 => kde_funcs::epanechnikov_kde::<4>(x, points, lamdaopt, n_threads),
        5 => kde_funcs::epanechnikov_kde::<5>(x, points, lamdaopt, n_threads),
        6 => kde_funcs::epanechnikov_kde::<6>(x, points, lamdaopt, n_threads),
        7 => kde_funcs::epanechnikov_kde::<7>(x, points, lamdaopt, n_threads),
...
fn epanechnikov_kde_py<'py>(
    ...
    let lamdaopt = lamdaopt.as_array();
    // let n_dim: usize = x.shape()[1];
    let res = kde_funcs::epanechnikov_kde_3d(x, points, lamdaopt, n_threads);
...
/*
#[pyfunction]
#[pyo3(name = "epanechnikov_kde_weights")]
fn epanechnikov_kde_weights_py<'py>(
    ...
    let res =
        kde_funcs::epanechnikov_kde_weights_3d(x, points, lamdaopt, n_threads, weights);
    res.to_pyarray(py)
}
...
/* #[pyfunction]
#[pyo3(name = "epanechnikov_kde_rev")]
fn epanechnikov_kde_rev_py<'py>(
    py: Python<'py>,
    x: PyReadonlyArray2<f64>,
    points: PyReadonlyArray2<f64>,
    lamdaopt: PyReadonlyArray1<f64>,
    ...
    let points = points.as_array();
    let lamdaopt = lamdaopt.as_array();
    let res = kde_funcs::epanechnikov_kde_rev(x, points, lamdaopt, n_threads);
    res.to_pyarray(py)
} */

// fn spyders(_py: Python, m: &PyModule) -> PyResult<()> {
#[pymodule]
fn spyders(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(epanechnikov_kde_py, m)?)?;
    /* m.add_function(wrap_pyfunction!(epanechnikov_kde_rev_py, m)?)?;
    m.add_function(wrap_pyfunction!(epanechnikov_kde_weights_py, m)?)?;
    m.add_function(wrap_pyfunction!(
        epanechnikov_kde_rev_weights_py,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        epanechnikov_kde_rev_weights_multi_py,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        epanechnikov_kde_rev_weights_groups_py,
        m */
    // )?)?;
    Ok(())
}
